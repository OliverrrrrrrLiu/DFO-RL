...
class DFO_GNCD(object):
	...
	"""
	def __init__(self, noise, r,ls_params,m, delta_max, delta_init, eta_1, eta_2, eta_3, ecn_params, output_level, rule,  his_len = 5, mode = "fd",  tol = 1e-6):
		"""
		...
		self.eta_3 = eta_3
		self.rule = rule
		self.c1, self.c2, self.max_iter = ls_params
	...
	def search(self, orig_pt, d, noise = 0.0, mode = "fd"):
		...
		while ls_counter < self.max_iter: #if max number of iterations has not been exceeded
			...
			self.eval_counter += 1
			condition_satisfied = self.is_armijo(orig_pt, f_trial, step, d, noise)
		...
		return (x_trial, f_trial), step, False
...
if __name__ == "__main__":
	...
	m = n
	x = initial_points(n,23)
	dfogncd = DFO_GNCD(1e-3,r_noise,(1e-4, 0.9, 20),m,100,1,0.1,1/4,3/4, (1e-8, 7, 100),3, rule = "random")

	_, _, _, fval_history, eval_history = dfogncd.run(x)
	plt.plot(eval_history,np.log(fval_history), label = "coordinate GN")
