...
class MaxHessian(object):
    ...
    def __init__(self, eps, tau_1 = 100, tau_2 = 0.1):
        """
    ...
    def calculate_delta(self, f, x, h, direction = None):
    ...
    def calculate_mu(self, f, x, mu = 1, direction = None):
        """Calculate the Hessian estimate
        @param mu: previous estimate of Hessian, to be used as a scalar (default: 1)
        """
        h = (self.eps / mu) ** 0.25
        dh, fx, fx_plus_h, fx_minus_h = self.calculate_delta(f, x, h, direction)
        mu = dh / h ** 2
    ...
    def estimate(self, f, x, direction = None):
        ...
        """
        h_a, dh_a, mu_a, fx, fx_plus_h_a, fx_minus_h_a = self.calculate_mu(f, x, direction = direction)
        if self.exit(h_a, dh_a, fx, fx_plus_h_a, fx_minus_h_a):
            return mu_a
        h_b, dh_b, mu_b, fx, fx_plus_h_b, fx_minus_h_b = self.calculate_mu(f, x, mu = mu_a, direction = direction)
...
def fd_gradient(f, x, eps,h = None, mode = "fd", tau_1 = 100, tau_2 = 0.1):
    mu = MaxHessian(eps, tau_1, tau_2).estimate(f, x, direction = None)
    dim = len(x)
...
if __name__ == "__main__":
    lst = []
    for _ in range(1000):
        tmp, _ = fd_gradient(f, np.array([5, 1]), 0.01)
