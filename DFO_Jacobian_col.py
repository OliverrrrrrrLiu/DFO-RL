			...
			if rho_k > self.eta_1:
				...
				if k % self.k_accept == 0:# or f_val <= 1e-3:
					...
					if self.rule == "random":
						...
						idx = np.random.choice(n,update_len, replace = False)
						for i in range(self.m):
							r_i = lambda x: self.r(x,self.m)[i] #i-th component of residual
							grad_i, _ = fd_gradient(r_i, x, self.noise, mode = self.mode)
							J[i][idx] = grad_i[idx]
						if self.mode == "fd":
							self.eval_counter += update_len
						else:
							self.eval_counter += 2 * update_len
					"""
					elif self.rule == "cyclic":
						idx_start = k % m 
						idx = np.array(range(self.j_len))
						idx = (idx + idx_start * self.j_len) % m
						for i in idx:
							r_i = lambda x: self.r(x,self.m)[i]
							grad_i, _ = fd_gradient(r_i, x, self.noise, mode = self.mode)
							J[i] = grad_i
							if self.mode == "fd":
								self.eval_counter += n/m
							else:
								self.eval_counter += 2*n/m		
					"""			
				
				"""
				if k % 10 == 0 or f_val <= 1e-3:
					for i in range(self.m):
						r_i = lambda x: self.r(x,self.m)[i] #i-th component of residual
						grad_i, _ = fd_gradient(r_i, x, self.noise, mode = self.mode) #compute gradient of r_i
						#if self.mode == "fd":
...
def r_noise(x,m):
	return fun20(x,m) + 1e-3 *np.random.normal()
...
if __name__ == "__main__":
	...
	m = n
	dfogn = DFO_LSTR_COL(1e-3,r_noise,m,100,1,0.1,1/4,3/4, (1e-8, 7, 100), 5,0,3, rule = "random", j_len = 100)
