...
#ONLY UPDATE SOME COLUMNS OF THE JACOBIAN
#CHECK LINEARITY OF LOCAL MODEL TO DETERMINE WHETHER THE UPDATE OF FULL JACOBIAN IS NECESSARY
...
class DFO_L(object):
	...
	def run(self, x):
		...
		np.random.seed(123)
		n = len(x) #dimension
		update_len = np.ceil(n * self.j_len / 100).astype(int)
		...
		update_counter = 1 #set default
		constant_counter = 0 #set default

		while not self.is_convergent(f_val, fval_history, norm_grad_k, self.tol,self.his_len) and k <=200:
			...
			#Compute the ratio of linear model
			pred_k_linear =  -np.dot(grad, p_k)
			rho_k_linear = ared_k/pred_k_linear

			if update_counter != 0 or constant_counter != 0:
				pass
			else:
				if rho_k_linear > 0.5:
					print("freeze")
					constant_counter = 5 #freeze
					update_counter = 0
				else:
					print("update")
					constant_counter = 0 #update
					update_counter = 5
			...
			if rho_k > self.eta_1:
				...
				if constant_counter == 0: #if k % self.k_accept == 0:# or f_val <= 1e-3:
					for i in range(self.m):
						r_i = lambda x: self.r(x,self.m)[i] #i-th component of residual
			...
			"""
				...
				if update_counter == 0: #if self.k_reject != 0 and k % self.k_reject == 0:# or f_val <= 1e-3:
						for i in range(self.m):
							r_i = lambda x: self.r(x,self.m)[i] #i-th component of residual
							grad_i, _ = fd_gradient(r_i, x, self.noise, mode = self.mode) #compute gradient of r_i
							#if self.mode == "fd":
...
if __name__ == "__main__":
	...
	m = n
	dfogn = DFO_L(1e-3,r_noise,m,100,1,0.1,1/4,3/4, (1e-8, 7, 100), 5,0,3, rule = "random", j_len = 100)
